...
public:
    ...
    LidarAvoidCmd() : Node("lidar_avoid_cmd") {
        ...
        this->declare_parameter<double>("angular_boost_factor_at_min_dist", 1.5); // min_avoidance_dist で旋回を何倍に増幅するか
        this->declare_parameter<double>("ttc_for_angular_boost", 1.0);       // [秒] このTTC以下で旋回増幅を開始する
        this->declare_parameter<double>("max_angular_boost_factor", 2.0);     // 旋回増幅の最大倍率
...
private:
    ...
    void publish_velocity() {
        ...
        if (!isWork_ || state_ == State::NORMAL) {
            ...
            double angular_boost_at_min = this->get_parameter("angular_boost_factor_at_min_dist").as_double();
            double ttc_for_boost = this->get_parameter("ttc_for_angular_boost").as_double();
            double max_angular_boost = this->get_parameter("max_angular_boost_factor").as_double();
            double linear_vel_threshold = 0.01; // 前進指令の閾値 (TTC計算用)
            ...
            double base_turn_strength_factor = (distance_threshold - closest_obstacle_dist_) / (distance_threshold - min_avoidance_dist + 1e-6);
            base_turn_strength_factor = std::clamp(base_turn_strength_factor, 0.0, 1.0);



            // 4b. 旋回増幅係数の計算
            double angular_boost_factor = 1.0; // デフォルトは増幅なし

            // 4b-1. 距離に基づく増幅 (min_avoidance_distに近づくほど強く)
            if (closest_obstacle_dist_ < distance_threshold) { // distance_thresholdより近い場合のみ考慮
                // distance_thresholdで1.0, min_avoidance_distで angular_boost_at_min になるように線形補間
                double dist_based_boost = 1.0 + (angular_boost_at_min - 1.0) *
                                           ((distance_threshold - closest_obstacle_dist_) / (distance_threshold - min_avoidance_dist + 1e-6));
                dist_based_boost = std::clamp(dist_based_boost, 1.0, angular_boost_at_min); // 1.0 から設定値の間
                angular_boost_factor = std::max(angular_boost_factor, dist_based_boost);
            }

            // 4b-2. TTCに基づく増幅 (TTCが短いほど強く)
            double ttc = std::numeric_limits<double>::infinity();
            if (cmd_vel_remote_.linear.x > linear_vel_threshold && std::abs(closest_obstacle_angle_) < (M_PI / 2.0) ) {
                double speed_towards_obstacle = cmd_vel_remote_.linear.x * std::cos(closest_obstacle_angle_);
                if (speed_towards_obstacle > 1e-3) {
                    ttc = closest_obstacle_dist_ / speed_towards_obstacle;
                    if (ttc < ttc_for_boost) {
                        // ttc_for_boostで1.0、TTCが0に近づくほどmax_angular_boostに近づく線形補間
                        // (より単純な実装として、TTC < ttc_for_boost なら一定のブーストをかけるなども考えられる)
                        double ttc_based_boost = 1.0 + (max_angular_boost - 1.0) * ((ttc_for_boost - ttc) / (ttc_for_boost + 1e-6));
                        ttc_based_boost = std::clamp(ttc_based_boost, 1.0, max_angular_boost);
                        angular_boost_factor = std::max(angular_boost_factor, ttc_based_boost);
                    }
                }
            }
            // 最終的な増幅係数は、最大値でクリップ
            angular_boost_factor = std::min(angular_boost_factor, max_angular_boost);
            ...
            double obstacle_avoidance_turn = -std::copysign(1.0, relative_angle_to_target + 1e-6) // ゼロ付近の不安定さ回避
                                             * avoidance_gain
                                             * base_turn_strength_factor
                                             * angular_boost_factor;
            ...
            // --- 6. 並進速度の計算 (変更なし: cmd_vel_remote_の値をそのまま使用) ---
            cmd_msg.linear.x = cmd_vel_remote_.linear.x;
